...
export class BasicController {
    ...
    try {
      ...
      if (validUser.password === null && validUser.googleId) {
        ...
        if (validUser) {
          ...
          // Email not found
          res.render("login", {
            errMsg: "Email does not exist! Please register to continue",
          });
        }
      }
    } catch (error) {
  ...
  //Function to get change password form for signed in user
  ...
  //Function to update password for signed in user
  ...
  //Function to get registered user email
  getOtpEmail = (req, res, next) => {
    res.render("getEmail", { errMsg: null });
  };

  //Function to get the otp page
  ...
  //Function to get the page after successful user login
  getHome = (req, res) => {
    res.render("home", {
      successMsg: null,
      useremail: req.cookies.email,
